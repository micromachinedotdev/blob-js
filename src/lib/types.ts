...
/**
 * Configuration options for R2 operations
 */
export interface R2Options extends BlobPropertyBag {
  /**
  ...
  /**
   * The R2 bucket name. Defaults to `R2_BUCKET` or `AWS_BUCKET` environment variables.
   *
   * @example
   ...
   *     // With R2_BUCKET=my-bucket in .env
   *     const file = s3.file("my-file.txt");
   */
  bucket?: string;

  /**
   * The AWS region. Defaults to `R2_REGION` or `AWS_REGION` environment variables.
   *
   * @example
   *     const file = s3.file("my-file.txt", {
   *       bucket: "my-bucket",
   ...
   * Defaults to `R2_ACCESS_KEY_ID` or `AWS_ACCESS_KEY_ID` environment variables.
   */
  accessKeyId?: string;

  /**
   * The secret access key for authentication.
   * Defaults to `R2_SECRET_ACCESS_KEY` or `AWS_SECRET_ACCESS_KEY` environment variables.
   */
  secretAccessKey?: string;

  /**
   * Optional session token for temporary credentials.
   * Defaults to `R2_SESSION_TOKEN` or `AWS_SESSION_TOKEN` environment variables.
   *
   * @example
  ...
  /**
   * The R2-compatible service endpoint URL.
   * Defaults to `R2_ENDPOINT` or `AWS_ENDPOINT` environment variables.
   *
   * @example
   *     // AWS R2
   *     const file = s3.file("my-file.txt", {
  ...
  /**
   * By default, Amazon R2 uses the STANDARD Storage Class to store newly created objects.
   *
   * @example
 ...
 */
export interface R2FilePresignOptions extends R2Options {
  /**
...
/**
 * Represents a file in an R2-compatible storage service.
 * Extends the Blob interface for compatibility with web APIs.
 *
 * @category Cloud Storage
 */
export interface R2File extends Blob {
  /**
  ...
  /**
   * Creates a new R2File representing a slice of the original file.
   ...
   * @returns A new R2File representing the specified range
   *
   * @example
   *  // Reading file header
   ...
   *     const uploadUrl = await file.presign({
   *       method: "PUT",
   *       expiresIn: 3600,
   *       type: "image/jpeg",
   *       acl: "public-read"
   ...
   *     });
   */
  presign(options?: R2FilePresignOptions): Promise<string>;
   ...
   * await file.unlink();
   */
  unlink: R2File["delete"];

  /**
   * Get the stat of a file in an R2-compatible storage service.
   *
   * @returns Promise resolving to R2Stat
   */
  stat(): Promise<R2Stats>;
}

export interface R2ListObjectsOptions {
  /** Limits the response to keys that begin with the specified prefix. */
  prefix?: string;
  /** ContinuationToken indicates to R2 that the list is being continued on this bucket with a token. ContinuationToken is obfuscated and is not a real key. You can use this ContinuationToken for pagination of the list results. */
  continuationToken?: string;
  /** A delimiter is a character that you use to group keys. */
  delimiter?: string;
  /** Sets the maximum number of keys returned in the response. By default, the action returns up to 1,000 key names. The response might contain fewer keys but will never contain more. */
  maxKeys?: number;
  /** StartAfter is where you want R2 to start listing from. R2 starts listing after this specified key. StartAfter can be any key in the bucket. */
  startAfter?: string;
  /** Encoding type used by R2 to encode the object keys in the response. Responses are encoded only in UTF-8. An object key can contain any Unicode character. However, the XML 1.0 parser can't parse certain characters, such as characters with an ASCII value from 0 to 10. For characters that aren't supported in XML 1.0, you can add this parameter to request that R2 encode the keys in the response. */
  encodingType?: "url";
  /** If you want to return the owner field with each key in the result, then set the FetchOwner field to true. */
  fetchOwner?: boolean;
}

export interface R2ListObjectContent {
   ...
   *
   * - Objects created by the PUT Object, POST Object, or Copy operation, or through the AWS Management Console, and are encrypted by SSE-R2 or plaintext, have ETags that are an MD5 digest of their object data.
...
export interface R2ListObjectsResponse {
  ...
  contents?: R2ListObjectContent[];
  /** If ContinuationToken was sent with the request, it is included in the response. You can use the returned ContinuationToken for pagination of the list response.  */
  continuationToken?: string;
  /** Causes keys that contain the same string between the prefix and the first occurrence of the delimiter to be rolled up into a single result element in the CommonPrefixes collection. These rolled-up keys are not returned elsewhere in the response. Each rolled-up result counts as only one return against the MaxKeys value. */
  delimiter?: string;
  /** Encoding type used by R2 to encode object key names in the XML response. */
  encodingType?: "url";
  ...
  /** The bucket name. */
  name?: string;
  /** NextContinuationToken is sent when isTruncated is true, which means there are more keys in the bucket that can be listed. The next list requests to R2 can be continued with this NextContinuationToken. NextContinuationToken is obfuscated and is not a real key. */
